...
//style for each field, including sutable animation for animation the placeholder

//when focused upon
const INPUT_STYLE: &str = "w-full h-12 bg-[#333333] pr-4 pl-6 text-white
    mt-6 outline-none focus:outline-none focus:pl-7 transition-all duration-1000";

const CANCEL_BUTTON_STYLE: &str = "mt-10 bg-[#555555] px-8 py-2 rounded text-white
...
const NO_ERROR_STYLE: &str = "flex flex-col bg-[#222222] border-t-8
    border-[#7734e7] px-6 pt-5 h-[29rem] w-full max-w-[36rem] z-50 -mt-2 fixed top-20
    z-50";

const ERROR_STYLE: &str = "flex flex-col bg-[#222222] border-t-8
    border-[#7734e7] px-6 pt-5 h-[32rem] w-full max-w-[36rem] z-50 -mt-2 fixed top-20
    z-50";
...
pub fn EditPersonModal(
    ...
    let on_click = move |_| {
        ...
        let validate_compensation = compensation().parse::<i32>();

        // if no issues with validating the compensation,
        if let Ok(ok_compensation) = validate_compensation {
            let edit_person_request = EditPersonRequest::new(uuid, person_title(), person_level(), ok_compensation);
                        ...
                        match edit_result {
                            ...
                            Err(_e) => {
                                ...
                                set_error_message(String::from(
                                    "Error updating the person. Please try again later.",
                                ))
                            }
                        };
                    });
    ...
    view! {
        <div class="flex flex-col w-full h-full z-50 mx-auto items-center align-center">
            <div class={ move || {
                ...
                <div class="flex flex-col w-full item-right justify-right mt-3">
